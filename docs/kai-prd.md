...
> 前提：本文描述的是一个**尚未实现**的库。不参考现有 Kai（s3ss10n）的 API 设计；仅吸收其使用方（Zafiro）验证过的需求与 Pi/Codex 验证过的架构模式。
...
[ ] 确认 Zafiro 层关于 Fork、对话消息方面的修改是否牵动 Kai 的 history 相关设计。Kai 是否支援树状数据结构等（Coding Agent 的对话在进行 fork 之后数据结构就是树）
[ ] 确认 Zafiro 持久化与 Protocol 切换相关的矛盾，是否存在矛盾等
...
6. **职责上溢**：MCP 刷新决策、回合串行化、资源清理时序等工作被迫由消费方（Zafiro）承担
...
**复用性目标**：同一定位可服务 语音助手 / 桌面 CLI / 测试桩 / 后续多模态与实时场景。衡量标准：**库不依赖任何 Zafiro 类型**；所有 host 特性通过接口、拦截器、配置注入。
...
- 消息模型对齐 pi：`User` / `Assistant` / `ToolResult` 三种具体消息类型，无通用角色+内容基类；`ToolResult` 是消息类型，不是 content block，其 `content` 为任意文本（工具结果不强制 JSON，Zafiro 现有工具即返回纯文本）；`ToolResult` 内嵌统一的 `ToolCallOutcome`（Success/Failure/Blocked/Interrupted/Unknown，拦截器链、执行器、历史共用同一类型，无状态映射），中断语义在持久化恢复后仍可读（超出 pi：pi 仅 content+isError，无 force-only stop）
...
- host（Zafiro）用拦截器实现：无障碍操作的"先读屏再执行"强制、自定义工具审批、运行中工具状态上报、失败重试策略——**这些现在散落在 Zafiro 工具实现里，统一收口到链上**
...
- **compaction 口子**：提供 `ContextPolicy` 接口（触发阈值、压缩回调），**M0 不实现**，host 层（Zafiro 提示词层）将来实现
...
## 5. 可从 Zafiro 下沉的逻辑（复用性收益清单）

| Zafiro 现状（决策/补偿逻辑） | 下沉到 Kai 的形式 |
|---|---|
...
| 里程碑 | 范围 | 验收 |
|---|---|---|
| **M0 — DeepSeek 跑通（本 PRD 的首要交付）** | 统一内容模型（text/thinking/toolCall/toolResult）· 流式事件协议 · DeepSeek 协议 + compat（thinking/reasoning_effort/max_tokens/回传策略）· Agent loop（tool 循环 + 段原子性）· **拦截器链骨架**（approval/rewrite/audit 三个内置通用拦截器）· 传输层重试 + 回合层段首重试 · 传输活跃度 idle · Session 序列化 | Zafiro 替换现有接入，DeepSeek 思考可见、工具循环带拦截器、429/5xx 自动退避、长思考不误杀；无回归 |
...
1. **可移植**：Kai 模块零依赖 Zafiro 类型；一个新 host（如桌面 CLI demo）只写提示词/工具/渲染即可复用
