...
/**
 * Experimental ChatGPT/Codex device-code flow. This remains separate from the
 * normal API-key provider and can be disabled independently if the upstream
 * device-auth endpoints change.
 */
class OpenAiDeviceAuthManager(
        ...
        withContext(Dispatchers.IO) {
            ...
            client.newCall(request).execute().use { response ->
                ...
                return@withContext when {
                    "authorization_pending" in error || "pending" in error -> OpenAiDevicePollResult.Pending
                    "access_denied" in error || "denied" in error -> OpenAiDevicePollResult.AccessDenied
                    "expired" in error -> OpenAiDevicePollResult.Expired
                    response.code == 404 || response.code == 408 || response.code == 429 -> OpenAiDevicePollResult.Pending
                    else -> OpenAiDevicePollResult.Failed("Device authorization failed (HTTP ${response.code}).")
                }
            }
        }
    ...
    private fun String.toJsonOrEmpty(): JSONObject = runCatching { JSONObject(this) }.getOrElse { JSONObject() }

    private fun JSONObject.requiredString(name: String): String =
        optNullableString(name) ?: throw IOException("OpenAI auth response is missing '$name'")
    ...
    private companion object {
        ...
        const val DEVICE_AUTH_USERCODE_URL = "https://auth.openai.com/api/accounts/deviceauth/usercode"
        const val DEVICE_AUTH_TOKEN_URL = "https://auth.openai.com/api/accounts/deviceauth/token"
...
/** Refresh tokens remain encrypted in [OpenAiTokenStore]; access tokens stay in memory. */
class OpenAiAuthRepository(
    ...
    suspend fun pollLogin(session: OpenAiDeviceCodeSession): OpenAiLoginResult {
        return when (val poll = manager.pollDeviceFlow(session)) {
            ...
            is OpenAiDevicePollResult.Authorized -> runCatching {
                val token = manager.exchangeAuthorizationCode(poll.authorizationCode, poll.codeVerifier)
                val identity = manager.identityFromIdToken(token.idToken)
                    ?: OpenAiAccountIdentity(UUID.randomUUID().toString(), null, null)
                val refreshToken = token.refreshToken
                    ?: return OpenAiLoginResult.Failed("Login succeeded but no refresh token was returned.")
                tokenStore.save(
                    StoredOpenAiAccount(
    ...
    fun currentAccount(): OpenAiAccountIdentity? {
        ...
        return OpenAiAccountIdentity(stored.localAccountId, stored.email, stored.chatgptAccountId)
    }
