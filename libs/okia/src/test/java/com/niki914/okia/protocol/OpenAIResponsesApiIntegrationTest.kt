...
/**
 * 门面 + 真实 DeepSeek /responses 端点集成测试（多协议 M1 层 2）。
 * 验证 OpenAIResponsesProtocol 对真实字节流的吻合（DeepSeek 提供 Responses
 * 兼容端点，2026-08-18 实测可用）：
 * - 命名事件流解析（response.output_text.delta / response.completed）
 * - input item 数组（role 消息 / instructions）
 * - usage 解析（input/output tokens）
 * key 从 OKIA_TEST_API_KEY 环境变量读取（运行测试时注入，不进仓库）；
 * 缺失时 Assume 跳过（不是失败）。
 */
class OpenAIResponsesApiIntegrationTest {
    ...
    @Test
    fun `real deepseek responses turn completes with text and usage`() = runBlocking {
        ...
        try {
            ...
            val result = okia.send("用一句话解释什么是 SSE。") { events += it }
            ...
            // usage 记账（response.completed 携带 usage）
