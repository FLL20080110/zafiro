...
### OKIA-1（待提）MCP 发现与 LLM loop 互不干涉：refreshMcpTools 不得与 send 互相阻塞
...
### OKIA-3（待提）`RealConversation.project(null)` 语义与文档不一致
...
### OKIA-4（待提）ToolRegistry「活跃回合不得直接变更 registry」注释与 update 路由矛盾
...
### OKIA-5（待提，低优先）send 前「确保 MCP 工具就绪」的统一入口
